...
<body>
	...
	<table class="table table-striped table-hover">
		<tr>
			...
			<td colspan="5" align="right">
				<form action="index.php" method="post">
					<input type="text" name="cr" placeholder="ID Petugas">
		...
		$data = mysqli_query($mysqli, "select * from tb_petugas");
		...
		if (isset($_POST['cari'])) {
			$idm = $_POST['cr'];
			if ($idm != "") {
				$sql = mysqli_query($mysqli, "SELECT * FROM tb_petugas where idPetugas='$idm' limit $halaman_awal, $batas");
			} else {
				$sql = mysqli_query($mysqli, "SELECT * FROM tb_petugas limit $halaman_awal, $batas");
			}
		} else {
			$sql = mysqli_query($mysqli, "SELECT * FROM tb_petugas limit $halaman_awal, $batas");
		}
		...
		?>
			<tr>
				...
				<td>
					<a class="edit" href="edit.php?id=<?php echo $data['idPetugas']; ?>">Edit</a> |
					<a class="hapus" href="delete.php?id=<?php echo $data['idPetugas']; ?>">Hapus</a>
				</td>
			</tr>
		<?php } ?>
	</table>
